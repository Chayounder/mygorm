...
/*  var tunnels []Tunnel
     err := testQueryTable(db, "id", &tunnels, nil)
    if err != nil {
        t.Log("QueryAllDevices error:", err)
    }
    fmt.Printf("counts:%d, %p\n", len(tunnels), &tunnels)
    fmt.Println(tunnels)*/

    devices, err := QueryAllDevices(db,"id")
    if err != nil {
        t.Log("QueryAllDevices error:", err)
    }
    fmt.Printf("counts:%d, %p\n", len(devices), &devices)
    ...
    /*
    fmt.Println("=============QueryAllDevicesBatches================")


    devices, err = QueryAllDevicesBatches(db, "id")
    if err != nil {
        t.Log("QueryAllDevicesBatches error:", err)
    }
    fmt.Printf("counts:%d, %p\n", len(devices), &devices)
    //for _, dev := range devices {
    //  fmt.Println(dev)
    ...
    fmt.Println("=============QueryDeviceTunnels================")
    device := &Device{Cid: originDevs[0].Cid}
    fmt.Println(device.Tunnels)
    err = device. QueryDeviceTunnels(db)
    if err != nil {
        t.Log("QueryAllDevices error:", err)
    }
    fmt.Println(device.Tunnels)
...
const (
    RunTime = 50 // 分钟
    DeviceNumMax = 4 * 25 * 1000 // total = 100 +
    RoutineNum = 100
    SleepFlag = 1
)
var ctlClient *ControlClient
var offClient *ControlClient
var stop int64
...
var gVendor = []string{"Tenda", "IP-COM", "HaiKang", "HuaWei"}
    ...
    go func() {
        ...
        for i := 0; i < DeviceNumMax && time.Now().Unix() < stop; i += itr {
            for j, ven := range gVendor {
                ...
                time.Sleep(time.Millisecond * 10 * SleepFlag)
            }
        }
...
func testQuery(t *testing.T, wg *sync.WaitGroup) {
    ...
    if len(ctlClient.controls) == 0 {
        time.Sleep(time.Second * 5)
    }

    for i := 0; i < RoutineNum / 4; i++ {
        qWg.Add(1)
        go func(sqWg *sync.WaitGroup) {
            defer sqWg.Done()
            for {
                if time.Now().Unix() > stop {
                    break
                }
                if len(ctlClient.controls) == 0 { // 没考虑加锁
                    continue
                }
                devices, err := QueryAllDevices(db, "")
                if err != nil {
                    t.Error(errors.New("QueryAllDevices error:" + err.Error()))
                    continue
                }
                t.Log(fmt.Errorf("QueryAllDevices: %p", &devices))
                time.Sleep(time.Second * 2 * SleepFlag)
            }
        }(&qWg)
    }
    ...
    for i := 0; i < RoutineNum / 4; i++ {
        ...
        go func(sqWg *sync.WaitGroup) {
            ...
            for {
                ...
                devices, err := QueryOnlineDevices(db, "")
                if err != nil {
                    t.Error(errors.New("QueryOnlineDevices error:" + err.Error()))
                    continue
                }
                t.Log(fmt.Errorf("QueryOnlineDevices: %p", &devices))
                time.Sleep(time.Second * 1 * SleepFlag)
            }
        }(&qWg)
    }
        ...
        go func(sqWg *sync.WaitGroup) {
            ...
            for {
                for _,  ven := range gVendor {
                    ...
                    if err != nil {
                        t.Error(errors.New("QueryDevicesByVendor error:" + err.Error()))
                        continue
                    }
        ...
        go func(sqWg *sync.WaitGroup) {
            ...
            for {
                ...
                for cid, _ := range ctlClient.controls {
                    ...
                    ctlClient.RUnlock()
                    if err != nil {
                        t.Error(errors.New("QueryDevicesByClientID error:" + err.Error()))
                        ctlClient.RLock()
                        continue
                    }
                    t.Log(fmt.Errorf("query devices by cid[%s]-sn[%s]", cid, device.SN))
                    time.Sleep(time.Second * 5 * SleepFlag)
                    ctlClient.RLock()
                }
                ctlClient.RUnlock()
                if time.Now().Unix() > stop {
...
func testUpdate(t *testing.T, wg *sync.WaitGroup) {
    ...
    for i := 0; i < RoutineNum / 2; i++ {
        uWg.Add(1)
        go func(suWg *sync.WaitGroup) {
            defer suWg.Done()
            for {
                for _, vendor := range gVendor {
                    ...
                    if err = device.UpdateDeviceByCID(db); err != nil {
                        t.Error(errors.New("UpdateDeviceByID" + err.Error()))
                        continue
                    }
                ...
                time.Sleep(time.Second * 10 * SleepFlag)
            }
        }(&uWg)
    }
    ...
    for i := 0; i < RoutineNum / 2; i++ {
        ...
        go func(suWg *sync.WaitGroup) {
            ...
            for {
                ...
                device := &Device{Cid:"", State: 365 + uint(mrand.Intn(365))}
                ctlClient.Lock()
                if len(ctlClient.controls) == 0 {
                    ctlClient.Unlock()
                    time.Sleep(time.Second * 5)
                ...
                num := mrand.Intn(len(ctlClient.controls)) // 随机让第num个离线天数超过365天
                idx := 0
                var control Control
                for cid, c := range ctlClient.controls {
                    if idx == num && c != nil {
                        if c.State && c.IsTunnel {
                            device.Cid = cid
                            control = Control{IsTunnel: c.IsTunnel, State: c.State}
                            break
                        }
                        num++
                    }
                ...
                ctlClient.controls[device.Cid].State = false
                ctlClient.Unlock()
                if err := device.UpdateDeviceStateByCID(db); err != nil {
                    t.Error(errors.New("UpdateDeviceStateByCID" + err.Error()))
                    continue
                }
                offClient.add(device.Cid, &control)
...
func testDelete(t *testing.T, wg *sync.WaitGroup) {
    ...
    if len(offClient.controls) == 0 {
        time.Sleep(time.Second * 10)
    }

    for i := 0; i < RoutineNum; i++ {
        dWg.Add(1)
        go func(sdWg *sync.WaitGroup) {
            defer sdWg.Done()
            var device Device
            for {
                if time.Now().Unix() > stop {
                ...
                device.Cid = offClient.firstGetDel()
                if len(device.Cid) == 0 {
                    time.Sleep(time.Second * 5)
                    continue
                }
                _ = ctlClient.del(device.Cid)
                if err := device.DeleteDeviceByCID(db); err != nil {
                    t.Error(errors.New("DeleteDeviceByCID err" + err.Error()))
                    continue
                }

                t.Log("id[",device.Cid,"] device deleted")
                time.Sleep(time.Second * 5 * SleepFlag)
            }
        }(&dWg)
    }
    dWg.Wait()
    t.Log("testDelete finished!")
}
...
func (c *ControlClient) firstGetDel() (clientID string) {
    c.Lock()
    defer c.Unlock()

    for cid, con := range c.controls {
        if con == nil || len(cid) == 0{
            continue
        } else {
            clientID = cid
            delete(c.controls, cid)
            return
        }
    }
    return ""
